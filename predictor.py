...
def resBlock(x, num_outputs, kernel_size = 4, stride=1, activation_fn=tf.compat.v1.nn.relu, normalizer_fn=tcl.batch_norm, scope=None):
    ...
    def __call__(self, x, is_training = True):
        with tf.compat.v1.variable_scope(self.name) as scope:
            with arg_scope([tcl.batch_norm], is_training=is_training, scale=True):
                with arg_scope([tcl.conv2d, tcl.conv2d_transpose], activation_fn=tf.compat.v1.nn.relu,
                    ...
                    pd = tcl.conv2d_transpose(pd, 3, 4, stride=1) # 256 x 256 x 3
                    pos = tcl.conv2d_transpose(pd, 3, 4, stride=1, activation_fn = tf.compat.v1.nn.sigmoid)#, padding='SAME', weights_initializer=tf.compat.v1.random_normal_initializer(0, 0.02))
...
class PosPrediction():
    def __init__(self, resolution_inp = 256, resolution_op = 256):
        ...
        # net forward
        self.x = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=[None, self.resolution_inp, self.resolution_inp, 3])
